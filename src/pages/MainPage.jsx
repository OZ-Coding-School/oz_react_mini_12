...
function MainPage() {
  ...
  if (loading === 'pending') {
    return <div className="text-center p-8 text-xl font-semibold text-gray-400">영화 정보를 로드 중입니다...</div>;
  }
  ...
  if (debouncedSearchTerm && movies.length === 0 && loading === 'succeeded') {
    return <div className="text-center p-8 text-gray-500 text-lg">"{debouncedSearchTerm}"(으)로 검색된 영화가 없습니다.</div>;
  }
